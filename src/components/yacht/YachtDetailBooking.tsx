...
const YachtDetailBooking = () => {
  ...
  const onSubmit = (data: BookingFormData) => {
    // Compose mailto with yacht detail booking data
    const subject = encodeURIComponent('New Yacht Charter Booking Request - Miami Elite Yacht');
    const body = encodeURIComponent(
      `Yacht: Miami Elite Yacht\nName: ${data.name}\nEmail: ${data.email}\nPhone: ${data.phone}\nGuests: ${data.guests}\nDate: ${selectedDate ? selectedDate.toLocaleDateString() : 'Not selected'}\nTime: ${data.time}\nDuration: ${data.duration} hours\nSpecial Requests: ${data.specialRequests || 'None'}`
    );
